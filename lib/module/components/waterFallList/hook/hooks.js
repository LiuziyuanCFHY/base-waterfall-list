...
import { GOOD_CARD_WIDTH, leftKey, ONE_LINE_TITLE_FONT, rightKey } from '../config/config';
import { useLatest } from '@locallife/base-hooks';
import { EStyleUtil } from '../../../utils/EStyleUtil';
export const useDoubleListData = (feedGoodsInfos, refreshCount = 0, cardWidth = GOOD_CARD_WIDTH) => {
  const leftHeight = useRef(0);
  const rightHeight = useRef(0);
  const [dataList, setDataList] = useState([]);
  const [leftData, setLeft] = useState([]);
  const [rightData, setRight] = useState([]);
  ...
  useEffect(() => {
    ...
    tmpFeedGoodsInfos.forEach((item, index) => {
      ...
      // 2、计算卡片高度
      const itemHeight = countFeedItemHeight(newItem, ONE_LINE_TITLE_FONT, cardWidth);
      // 3、构造双列的数据源
  ...
  const updateHeight = useCallback((listKey, height) => {
    if (listKey === leftKey) {
...
export const useTextLineHook = title => {
