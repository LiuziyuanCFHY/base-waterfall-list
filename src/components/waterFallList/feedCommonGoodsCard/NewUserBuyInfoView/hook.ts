...
export const useBuyInfoSaleCount = (saleCount?: number) => {
    ...
    if (count < 100) {
        ...
        saleCountStr =
            '已抢' + (Math.round(count / 1000) / 10).toFixed(0) + '万+';
    }
    return saleCountStr;
};

export function removeEndValue(value: string) {
    if (value.endsWith('.0')) {
...
export function calculateSoldCount(value: string | number) {
